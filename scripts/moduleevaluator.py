...
class ModuleEvaluator:
    ...
    def _apply_range_augment_effect_on_tile(self, tilemodel : Tile) -> None:
        ...
        if tilemodel.range_attacks_power is not None:
            ...
            [range_attack_power + 1 for range_attack_power in tilemodel.range_attacks_power]
            )
    ...
    def _apply_scoper_effect_on_tile(self, tilemodel: Tile) -> None:
        if tilemodel.kind == "module":
            ...
            tilemodel.army_name = next_element(self.board.armies, tilemodel.army_name)
    ...
    def _get_prio_army_modules(self):
        modules_prio = []
    ...
    def _get_non_prio_army_modules(self):
        modules = []
        for army_name in self.board.armies:
            tiles = self.board.tiles[army_name]
            for tile in tiles:
    ...
    def _apply_prio_effect_modules(self):
        modules_prio = self._get_prio_army_modules()
        list_effects_prio, list_effects_position_prio = self._get_effects_modules(modules_prio)
        for index, effect_prio in enumerate(list_effects_prio):
            effect_position_prio = list_effects_position_prio[index]
            tile = self.board.get_tile_by_position(effect_position_prio)
            if tile is not None:
                self._apply_one_effect_on_tile(tile, effect_prio)

    def _apply_non_prio_effect_modules(self):
        modules_non_prio = self._get_non_prio_army_modules()
        list_effects_non_prio, list_effects_position_non_prio = self._get_effects_modules(modules_non_prio)
        for index, effect_non_prio in enumerate(list_effects_non_prio):
            ...
            tile = self.board.get_tile_by_position(effect_position_non_prio)
            if tile is not None:
                self._apply_one_effect_on_tile(tile, effect_non_prio)

    def apply_all_effect_modules(self):
        self._apply_prio_effect_modules()
        self._apply_non_prio_effect_modules()
    ...
    def apply_active_module_effect(self):
        ...
        for module in modules:
            if module.id_tile == "hegemony-transport":
                list_effects, list_effects_position= self._get_effects_modules([module])
                for index, effect in enumerate(list_effects):
                    effect_position = list_effects_position[index]
                    tile = self.board.get_tile_by_position(effect_position)
                    if tile is not None:
                        tile.module_effects.append("transport")
                        tile.special_capacities.append("movement")