...
class BattleEvaluator:
    ...
    Attributes
    ----------
        board (HexBoard): The game board containing tiles and their positions.
        module_evaluator (ModuleEvaluator): Evaluates and applies module effects.
        view (View): Provides a graphical interface for user interactions and feedback.
    ...
    ----------
        ...
        generate_attacks_with_quartiermaitre(tilemodel: Tile, event_list):
        ...
        run_battle(event_list):
            Simulates the entire battle sequence by resolving rounds in descending initiative order.
    ...
    ----------
        ...
        _process_cac_attack(tilemodel: Tile, enemy_army_name: str, cac_attack_direction: Tuple[int, int, int], cac_attack_power: int):
        ...
        _process_range_attack(tilemodel: Tile, enemy_army_name: str, range_attack_direction: Tuple[int, int, int], range_attack_power: int):
        ...
        _get_pixel_positions_cac_attacks(tilemodel: Tile) -> List[Tuple[int, int]] | None:
        ...
        _prompt_and_get_attacke_direction_convertion(tilemodel: Tile, attack_pixel_positions: List[Tuple[int, int]], event_list) -> Tuple[int, int, int] | None:
            Prompts the user to select an attack direction to convert and returns the chosen direction.

        _converte_attacks_and_copy_tile(tilemodel: Tile, range_attack_to_converte, cac_attack_to_converte) -> Tile:
    ...
    def generate_attacks_with_quartiermaitre(self, tilemodel: Tile, event_list: List[pygame.event.Event]) -> None:
        """Handles special attacks triggered by the "quartiermaître" module effect.
    ...
    def battle_round(self, initiative_round: int, event_list: List[pygame.event.Event]) -> None:
        """Executes a single round of combat for all tiles with the same initiative.
...
# --- MÉTHODES PRIVÉES ---
    ...
    def _get_pixel_positions_cac_attacks(self, tilemodel: Tile) -> List[Tuple[int, int]] | None:
        ...
        Returns:
            List[Tuple[int, int]] | None: List of pixel positions or None if the tile had any CQC attacks
        """
        if tilemodel.range_attacks_direction:
    ...
    def _get_pixel_positions_range_attacks(self, tilemodel: Tile) -> List[Tuple[int, int]] | None:
        ...
        Returns:
            List[Tuple[int, int]] | None: List of pixel positions or None if the tile had any ranged attacks
        """
    ...
    def _prompt_and_get_attacke_direction_convertion(self,
                                        ...
                                        event_list: List[pygame.event.Event]) -> Tuple[int, int, int] | None:
        """Prompts the user to select an attack direction to convert and returns the chosen direction.

        Args:
            tilemodel (Tile): Tile for which to convert an attack .
            attack_pixel_positions (List[Tuple[int, int]]): List of the pixel position attacks of the Tile 'tilemodel'.
    ...
    def _converte_attacks_and_copy_tile(self,
                                        ...
                                        cac_attack_to_converte: Tuple[int, int, int] | None) -> Tile:
        """Creates a copy of the tile and converts a specified attack.

        Args:
            tilemodel (Tile): Tile to copy and convert
            range_attack_to_converte (Tuple[int, int, int] | None): Ranged attack to convert. None if any
            cac_attack_to_converte (Tuple[int, int, int] | None): CQC attack to convert. None if any
